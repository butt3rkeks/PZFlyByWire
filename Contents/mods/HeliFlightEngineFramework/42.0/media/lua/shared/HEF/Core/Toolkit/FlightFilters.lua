--[[
    FlightFilters — Composable filter pipeline for horizontal thrust

    All pure stateless functions. No state, no game API, no config reads.
    Parameters passed in by caller (engine facade).

    Pipeline: noiseFloor → thrustDecomposition → speedClamp → directionClamp

    Moved from Engines/FBW/FBWFilters.lua to Core/Toolkit/ — engine-agnostic.
]]
...
function FlightFilters.applyNoiseFloor(totalTiltRad, noiseFloor)
...
function FlightFilters.decomposeThrustDirection(pitchDev, rollDev, totalTiltRad, fwdX, fwdZ, maxHSpeed, effectiveTilt)
...
function FlightFilters.clampSpeed(velX, velZ, maxHSpeed)
...
function FlightFilters.clampThrustDirection(velX, velZ, totalSpeed, moveAngle, moveMag, maxLeadDeg)
