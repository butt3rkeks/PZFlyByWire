--[[
    SimController — Per-frame reference trajectory management
    ...
    Operates on SimModel2D and ErrorTracker2D instances passed in by the engine
    (does not own them — engine also needs them for error tracking, debug,
    ...
    Moved from Engines/FBW/FBWSimController.lua to Core/Toolkit/.
    Refactored: heading reanchor check is now a callback parameter instead of
    calling FBWYawController directly. This makes the module engine-agnostic.
]]
...
function SimController.resolveDesiredVelocity(hasHInput, totalVelX, totalVelZ,
...
--
-- @param checkHeadingReanchor function|nil Callback: checkHeadingReanchor(fps) → boolean.
--        Called to determine if heading reanchor should fire. Each engine provides
--        its own yaw controller's check function. Pass nil to skip reanchor entirely.
-------------------------------------------------------------------------------------
...
--- @param desiredHX number Desired horizontal velocity X
--- @param desiredHZ number Desired horizontal velocity Z
...
--- @param hasHInput boolean Whether horizontal input is active
--- @param posX number Current actual position X
--- @param posZ number Current actual position Z
--- @param fps number Current frame rate
--- @param flightAssistOff boolean FA-off mode active
--- @param positionDeltaSpeed number Position delta speed from framework
--- @param checkHeadingReanchor function|nil Engine-specific reanchor check callback
function SimController.advanceAndAnchor(sim, errorTracker, desiredHX, desiredHZ,
        deltaTime, effectiveInertia, hasHInput, posX, posZ, fps, flightAssistOff, positionDeltaSpeed,
        checkHeadingReanchor)
    ...
    if not flightAssistOff and checkHeadingReanchor then
        if checkHeadingReanchor(fps) then
...
FBWSimController = SimController
