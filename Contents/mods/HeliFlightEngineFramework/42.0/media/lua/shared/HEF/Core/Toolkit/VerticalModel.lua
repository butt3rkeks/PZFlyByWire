...
--- @param currentAltitude number Current altitude (z-levels)
...
function VerticalModel.computeTarget(keys, velY, currentAltitude, fuelPercent, engineCondition, freeMode, cfg)
    ...
    if keys.s and currentAltitude > cfg.minDescentAltitude then
        return -cfg.descendSpeed, true, false, false
    end

    -- 3. No fuel — gravity fall
    if fuelPercent <= 0 and currentAltitude > cfg.minDescentAltitude then
        return -cfg.gravityFallSpeed, false, false, false
    end

    -- 4. W key — ascend (with ceiling taper)
    if keys.w and currentAltitude < cfg.maxAltitude and fuelPercent > 0 then
        local speed = cfg.ascendSpeed
        local ceilingStart = cfg.maxAltitude - cfg.ceilingZoneHeight
        if currentAltitude > ceilingStart then
            local ceilingFactor = (cfg.maxAltitude - currentAltitude) / cfg.ceilingZoneHeight
