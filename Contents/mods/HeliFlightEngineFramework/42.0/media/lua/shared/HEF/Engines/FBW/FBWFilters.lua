--[[
    FBWFilters — Composable filter pipeline for horizontal thrust

    All pure stateless functions. No state, no game API, no config reads.
    Parameters passed in by caller (FBWEngine facade).
...
function FBWFilters.applyNoiseFloor(totalTiltRad, noiseFloor)
...
function FBWFilters.decomposeThrustDirection(pitchDev, rollDev, totalTiltRad, fwdX, fwdZ, maxHSpeed, effectiveTilt)
...
function FBWFilters.clampSpeed(velX, velZ, maxHSpeed)
...
function FBWFilters.clampThrustDirection(velX, velZ, totalSpeed, moveAngle, moveMag, maxLeadDeg)
