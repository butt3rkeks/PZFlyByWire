--[[
    HeliYawController — Yaw MPC (simYaw tracking + heading re-anchor)
...
--- Reset all yaw state.
function HeliYawController.reset()
    _simYaw = nil
    _wasRotating = false
    _lastSimYaw = nil
end
...
--- @param currentYawDeg number Current yaw from HeliOrientation (degrees)
--- @param isRotating boolean Whether A/D keys are pressed
--- @param yawDelta number Yaw change this frame (degrees, from HeliInputProcessor)
--- @return number simYaw The intended heading for hard lock
function HeliYawController.update(currentYawDeg, isRotating, yawDelta)
    if isRotating then
        if _simYaw then
...
function HeliYawController.checkHeadingReanchor(fps)
...
function HeliYawController.getSimYaw()
    return _simYaw
end
