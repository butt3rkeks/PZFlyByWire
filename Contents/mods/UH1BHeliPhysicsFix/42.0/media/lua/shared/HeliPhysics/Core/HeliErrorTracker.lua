--[[
    HeliErrorTracker — History buffer + tanh error saturation
...
function HeliErrorTracker.reset()
...
function HeliErrorTracker.record(actualX, actualZ, simPosX, simPosZ)
...
function HeliErrorTracker.getPositionError()
...
function HeliErrorTracker.getErrorMagnitude()
    local errX, errZ = HeliErrorTracker.getPositionError()
    return math.sqrt(errX * errX + errZ * errZ)
end

--- Clear history (used after heading re-anchor when old data is stale).
function HeliErrorTracker.clearHistory()
    _historyCount = 0
end
