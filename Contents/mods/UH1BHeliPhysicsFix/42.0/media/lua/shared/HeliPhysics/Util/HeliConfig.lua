--[[
    ...
    Runtime overrides via /hpf commands write here. No tunable lives outside HeliConfig.
...
local PARAMS = {
    ...
    -- Runtime-only (session overrides, from /hpf commands)
...
-- Ordered list for consistent display in /hpf show
local PARAM_ORDER = {
...
-- Runtime overrides (session-only, from /hpf commands)
local _overrides = {}
...
function HeliConfig.get(shorthand)
    -- Runtime override takes priority (session-only, from /hpf commands)
    ...
    if p and p.field and SandboxVars.HeliPhysics then
        local val = SandboxVars.HeliPhysics[p.field]
