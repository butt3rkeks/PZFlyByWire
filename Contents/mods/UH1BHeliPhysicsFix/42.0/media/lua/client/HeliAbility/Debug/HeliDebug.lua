--[[
    ...
    - One-shot snap capture (triggered by /hpf snap)
    - Last-frame state storage (read by /hpf snap in HeliDebugCommands)
    - Flight data recorder: writes per-frame CSV to Zomboid directory for offline analysis

    Controlled by: /hpf log, /hpf snap, /hpf record (in HeliDebugCommands.lua)
...
-- Last-frame state (written by orchestrator, read by /hpf snap command)
HeliDebug.lastState = {}
...
function HeliDebug.periodicLog(state, curr_z, desiredHX, desiredHZ, simVX, simVZ, errX, errZ, savedVelY, keyStr)
    ...
    if HeliDebug.lastLogTick >= HeliDebug.LOG_INTERVAL then
        HeliDebug.lastLogTick = 0
        print(string.format(
            "[HPF] %s z=%.2f des=(%.1f,%.1f) simV=(%.1f,%.1f) err=(%.2f,%.2f) svy=%.1f keys=%s",
            state, curr_z,
            desiredHX, desiredHZ,
            simVX, simVZ,
            errX, errZ,
...
function HeliDebug.logEngineOff(curr_z, fallSpeed, cvx, cvy, cvz)
    ...
    if HeliDebug.lastLogTick >= HeliDebug.LOG_INTERVAL then
        HeliDebug.lastLogTick = 0
        print(string.format("[HPF] ENG_OFF z=%.2f tgt=(0,%.1f,0) vel=(%.1f,%.1f,%.1f)",
...
function HeliDebug.startRecording()
    ...
    if not writer then
        print("[HPF] ERROR: Could not open " .. filename .. " for writing")
        return nil
    end
    ...
    print("[HPF] Recording started: " .. filename)
    return filename
end
...
function HeliDebug.stopRecording()
    ...
    print("[HPF] Recording stopped: " .. tostring(filename) .. " (" .. count .. " frames)")
    return count
end
...
---   yawSim    — simulation yaw (degrees, _simYaw from HeliRotation)
